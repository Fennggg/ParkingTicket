...
class ParkedCar{
    private:string make;
    public:const string &getMake() const {
        return make;
    }
    void setMake(const string &make) {
        ParkedCar::make = make;
    }
    public:const string &getModel() const {
        return model;
    }
    void setModel(const string &model) {
        ParkedCar::model = model;
    }
    private:string model;string licenseNumber;
    public:const string &getLicenseNumber() const {
    ...
    public:ParkedCar(string m,string mo,string ln,int mi){
        make =m;
        model=mo;
        licenseNumber =ln;
        minute = mi;
...
class ParkingMeter{
private:int minutePurchased;
public:int getMinutePurchased() const {
...
class ParkingTicket{
    ...
    private:int fine =0;bool noticket= true;
    public:
...
class PoliceOfficer
    ...
    public:PoliceOfficer(string n,int nu){
        name = n;
        number =nu;
    }
...
int main()
{
    ParkedCar c("car","phone","vehicle",500);
    ParkingMeter m;
    ParkingTicket t;
    m.setMinutePurchased(10);
    PoliceOfficer p("Mike",220);
